...
class MouseDragViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def distribution(self, request, **kwarg):
        ...
        id_drags_map = {f'{id_}': list(drags)
                        for id_, drags in groupby(drags.order_by('person__box__id'),
                                                  lambda drag:drag.person.box.id 
                                                  if drag.person is not None else 0)}
        response = []
...
class TeacherViewSet(viewsets.ModelViewSet):
    ...
    def distribution(self, request, **kwarg):
        ...
        teachers = InferenceModel(pk=model_pk).teachers \
                                              ...
                                              .order_by('label')
        label_teachers_map = [TeacherSerializer(v, many=True).data
    ...
    def add(self, request, **kwarg):
        model_pk = self.kwargs.get("model_pk")
        teachers = InferenceModel(pk=model_pk).teachers
        people = Person.objects.exclude(id__in=teachers.values_list('person__id', flat=True),
