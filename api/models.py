...
class BoundingBox(models.Model):
    ...
    def center_x(self) -> float:
        return float(self.xmax - self.xmin) / 2.0 + float(self.xmin)

    @property
    def center_y(self) -> float:
        return float(self.ymax - self.ymin) / 2.0 + float(self.ymin)
...
class Keypoints(UUIDModel):
    ...
    def _get_distance(self, point_1: Point, point_2: Point) -> float | None:
        if point_1.p == 0.0 or point_2.p == 0.0:
            return None
        return np.linalg.norm(np.array([point_1.x - point_2.x, point_1.y - point_2.y]))
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["frame", "group"], name="group_frame_unique"
            )
        ]
    ...
    def get_visualized_image(
        ...
        for person in self.people:
            cv2.putText(
                img,
                f"ID:{person.box.id}",
                (person.box.xmin, person.box.ymin),
            ...
            cv2.rectangle(
                img,
                (person.box.xmin, person.box.ymin),
                (person.box.xmax, person.box.ymax),
                color,
                thickness,
            )
            if draw_keypoints:
...
class Person(UUIDModel):
    ...
    def img(self) -> np.ndarray:
        ...
        img = frame_img[
            (self.box.ymin if self.box.ymin >= 0 else 0) : (
                self.box.ymax if self.box.ymax <= screen_height else screen_height
            ),
            (self.box.xmin if self.box.xmin >= 0 else 0) : (
                self.box.xmax if self.box.xmax <= screen_width else screen_width
            ),
        ]
        return img
    ...
    def get_visualized_img(
        ...
        for point in self.keypoints.points:
            cv2.circle(
                img_copy,
                (int(point.x) - self.box.xmin, int(point.y) - self.box.ymin),
                point_radius,
                color,
                thickness,
            )
